...
**幂等表是重试保护，不是索引级约束，而且会过期。** `prune_expired` 会删掉超过 `IDEMPOTENCY_RETENTION_DAYS`（30 天）的认领，所以「同一局只有一条记录」这个保证只在该局首次入库后的 30 天内成立。过了这个窗口再导入一份含旧对局的归档，每一局都会再存一遍：新的 `record_id`、新的 `received_at`，而且下游没有任何东西会把这两行合并——ReplacingMergeTree 做不到，重新入库的行在排序键上除了日期以外每一列都不同。两行都合法、都会被命中的筛选返回、什么都不会报错。

这是刻意的取舍：幂等表不能永久保存数亿行。要的如果是永久去重，那必须做在索引里（把对局 uuid 变成可查的列），不能靠这张表——见 issue #46。
