...
**幂等表里有两种认领，寿命不同**，因为它们回答的不是同一个问题。

调用方自己给的键（`source + Idempotency-Key`）是重试保护：只有那个调用方能重新算出它，超过重试窗口（30 天，`IDEMPOTENCY_RETENTION_DAYS`）就没有意义，到期删掉。

按对局 uuid 算出来的键**永不过期**。对局 uuid 不是索引的字段，所以这条认领是全系统唯一记着「这一局已经存过」的地方——删掉它不是释放一个保护，是把答案丢了：下一次导入含这局的归档就会用新的 `record_id` 再存一遍，而 `received_at` 和 `source` 都排在 `record_id` 前面，ReplacingMergeTree 合不了这两行。两行都合法、都会被筛选返回、什么都不报错。

代价是每局一行、永久保留。这是不过期去重能有的最省的形式：目前 810,004 局约 130MB，对应的是它保护的 25GiB 牌谱。`prune` 只删 `expires` 为真的那些，索引也做成了对应的 partial index，否则规模上去之后每次启动都要把永久行整个走一遍。

索引建立之前入库的记录没有这条认领，采集器留下的那些又是按过期写的，所以有一个一次性回填（`backfill::write_game_scoped_claims`，标记名 `game_scoped_claims`）：扫一遍索引，读每条记录的字节取出 uuid，补写或就地升级成永久认领。读不全就不写完成标记，下次启动重跑——读不到的那条记录，它的对局身份就是没被记下来。
...
**桶的键是两边都要拼出来的字符串。** ClickHouse 侧是 `toString(toDate(...))` / `toString(toStartOfHour(...))`，Rust 侧要逐字复现同样的格式再去 join——差一个 `T` 就是每个桶都查不到、全页归零，而且哪儿都不报错。给前端的 `at` 是另一套：小时用 RFC 3339 好让浏览器按读者本地时区渲染（把 05:00 显示给一个表上是 13:00 的人，比不标还糟），天保持裸日期，因为把它当成时刻解析会在格林尼治以西整体错开一天。
...
| broker 连接 | `Kafka::producers` / `Kafka::consumers` | 每个 partition 开**两个** `PartitionClient`，一个只用来 produce、一个给 pack worker 和 `sample_lag`。rskafka 一个 `PartitionClient` 就是一条 TCP，而 broker 在一个请求处理完之前不会读同一条连接上的下一个请求；pack worker 的 fetch 会挂在长轮询里最多 `FETCH_MAX_WAIT_MS`，共用连接时 produce 就一直躺在 socket 里没人读——哪怕它带的正是能满足那个 fetch 的记录。实测代价是入库被压到 30 条/秒，而 broker 报告每个 produce 只花 0.3ms。多开一条连接就没有这个耦合。 |
| topic 保留 | `redpanda-config` sidecar（compose） | `retention_bytes` / `log_retention_ms` 是 cluster property，进不去容器命令行（`--set` 只写进 redpanda.yaml，broker 会忽略并留一行日志）。原来放在 `.bootstrap.yaml`，但那个文件只在集群**首次成型**时读一次：本部署的数据卷 2026-07-27 09:35 建好，bootstrap 挂载当晚 22:20 才加进 compose，于是从没被读过——仓库里写着 40GiB 按大小裁，broker 实际跑的是不限大小、7 天按时间裁，整整一个月没有任何地方报出这个差别。现在改成每次启动跑一个 sidecar 调 admin API（`rpk cluster config set`，幂等），和 `create-bucket` 同一个套路：必须为真、又不能指望数据卷状态的东西，不放在只读一次的文件里。按 641,475 局历史导入、单条 p50 53,668 字节估算约 34.4GB，配 40GiB 上限；实际远低于此，producer 用 zstd 压过，实测 150,114 条只占 510MB。 |
