...
- 历史牌谱补抓：转换器修好之前入库的记录既没有原始 PB、mjai 也带着已修掉的那些错。它们都带 `majsoul.uuid`，所以可以按 uuid 重新抓一遍、用当前转换器重转、连 PB 一起写进新 pack 并**替换**索引里那一行（`record_id`/`source`/`received_at` 原样保留，靠 ReplacingMergeTree 收敛）。抓取不另开会话——同一账号重复登录会把采集踢下线——而是向正在跑的采集实例借：实例只在自己轮询间隙、本该睡过去的那段时间里应答，所以补抓永远不会让实时采集排队。只有当重转结果能解析、且事件数不少于原记录时才替换。
